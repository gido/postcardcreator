...
/**
 ...
 @param {Postcard} postcard object
 @param {function} callback (optional)
 */
Client.prototype.sendPostcard = function (postcard, callback) {
    ...
    client.users.current(function(err, user) {
        ...
        client.users.mailings.create(userId, {}, function (err, result) {
            ...
            // upload assets
            client.users.assets.create(userId, { asset: postcard.image }, function (err, result) {
                ...
                // set recipients
                client.users.mailings.recipients.put(userId, mailingId, postcard.recipient, function(err, result) {
                    ...
                    var sender = user;
                    sender.street = user.address; // to be more API compliant

                    var frontLayout = postcard.renderFrontPage({ assetId: assetId });
                    var backLayout = postcard.renderBackPage({ sender: sender });
...
Client.prototype.getBaseUrl = function () {
    ...
    if (this.user && this.password) {
        authPart = this.user + ':' + this.password + '@';
    }

    return (this.useSSL ? 'https':'http') + '://' + authPart + this.host + '/rest/1.0';
};
